...
pub async fn run(session_id: String, jwt: String) {
    println!("🎤 Starting macOS CoreAudio capture...");
    println!("Session ID: {}", session_id);
    
    CAPTURING.store(true, Ordering::Relaxed);
    
    // Initialize macOS CoreAudio capture
    ...
    tokio::spawn(async move {
        ...
        while capture_flag.load(Ordering::Relaxed) && CAPTURING.load(Ordering::Relaxed) {
            ...
            // 1. Use CoreAudio to capture system audio from BlackHole
        ...
        println!("🛑 macOS audio capture stopped");
    });
...
pub fn stop() {
    println!("🛑 Stopping macOS audio capture...");
