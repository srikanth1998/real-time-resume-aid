...
pub async fn run(session_id: String, jwt: String) {
    println!("🎤 Starting Linux PulseAudio capture...");
    println!("Session ID: {}", session_id);
    
    CAPTURING.store(true, Ordering::Relaxed);
    
    // Initialize Linux PulseAudio capture
    ...
    tokio::spawn(async move {
        ...
        while capture_flag.load(Ordering::Relaxed) && CAPTURING.load(Ordering::Relaxed) {
            ...
            // 1. Use PulseAudio to capture system audio
        ...
        println!("🛑 Linux audio capture stopped");
    });
...
pub fn stop() {
    println!("🛑 Stopping Linux audio capture...");
