...
const Interview = () => {
  ...
  const initializeSpeechRecognition = () => {
    ...
    recognitionRef.current.onresult = (event: any) => {
      ...
      // Generate answer when we have a complete sentence
      if (finalTranscript.trim() && (finalTranscript.includes('?') || finalTranscript.length > 20)) {
  ...
  const generateAnswer = async (question: string) => {
    ...
    try {
      ...
      // that processes the question with the uploaded documents and generates an answer
      const mockAnswer = `Based on your resume and the job requirements, here's a tailored response: ${question.slice(0, 50)}... [This would be generated by AI based on your documents]`;
      ...
      await supabase
        ...
        .insert({
          ...
          generated_answer: mockAnswer
        });

    } catch (error) {
      console.error('Answer generation error:', error);
      setCurrentAnswer("Sorry, there was an error generating the answer. Please try again.");
    } finally {
            ...
            <CardContent>
              <div className="bg-gray-900 p-4 rounded-lg min-h-[100px]">
                ...
                <p className="text-white">
                  {currentTranscript || (isListening ? "Listening..." : "Click 'Start Listening' to begin")}
                </p>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent>
              <div className="bg-gray-900 p-4 rounded-lg min-h-[200px]">
                <p className="text-white leading-relaxed">
                  {currentAnswer || "Ask a question to generate an AI-powered answer based on your resume and job description."}
                </p>
                {isGeneratingAnswer && (
