...
$label.Text = "PRIVACY OVERLAY`r`nDrag to move`r`nPress ESC to exit"
...
$transparencyMenu = $contextMenu.Items.Add("Transparency")
...
# Add color options
$colorMenu = $contextMenu.Items.Add("Color")
...
$protectionToggle.Click += {
    ...
    if ($script:isProtected) {
        ...
        $label.Text = "PRIVACY OVERLAY`r`nDrag to move`r`nPress ESC to exit`r`n(PROTECTED FROM SCREEN CAPTURE)"
    } else {
        $protectionToggle.Text = "Enable Screen Sharing Protection"
        $value = 0
        $result = [DwmApi]::DwmSetWindowAttribute($form.Handle, [DwmApi]::DWMWA_CLOAK, [ref]$value, 4)
